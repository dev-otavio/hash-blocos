    ...
    while (i < tamanho) {
        try {
            ...
            if (a1[i].length !== a2[i].length) {
                throw new Error(
                    `Arrays têm dimensões distintas na linha ${i}: (${a1[i].length} coluna(s), ${a2[i].length} coluna(s))`
                );
            } else {
                throw e;
            }
...
export function copiarArray2D(
    ...
    regiao: { linhas: number; colunas: number; linha: number; coluna: number }
): Array<Array<number>> {
    ...
    while (c < original.length) {
        if (original[c].length !== comprimento) {
            throw new Error(
                `${original} não representa uma matriz pois número de colunas da linha ${c} é diferente das linhas anteriores que têm ${comprimento} colunas.`
            );
        }
        c += 1;
    }
    ...
    if (linha + linhas > original.length) {
        throw new Error(
            `Valor inválido ${linhas}, apenas ${
                original.length - linha
            } linhas disponíveis para seleção a partir da linha ${linha}`
        );
    }

    if (coluna + colunas > original[linha].length) {
        throw new Error(
            `Valor inválido ${colunas}, apenas ${
                original[linha].length - coluna
            } colunas disponíveis para seleção a partir da coluna ${coluna}`
        );
    }
 ...
 */
export function duplicarArray2D(
    original: Array<Array<number>>
): Array<Array<number>> {
...
export function sobreporArray2D(
    ...
    if (a1.length - linha < a2.length) {
        throw new Error(
            `Argumento inválido ${a2}, espaço disponível para ${
                a1.length - linha
            } linha(s) a partir de linha ${linha}.`
        );
    }
    ...
    if (a1[0].length - coluna < a2[0].length) {
        throw new Error(
            `Argumento inválido ${a2}, espaço disponível para ${
                a1[0].length - coluna
            } coluna(s) a partir de linha ${linha} e coluna ${coluna}.`
        );
    }
...
export function reduzir2D(
    ...
    if (numeroElementos < 2) {
        throw new Error(
            numeroElementos ? `Array possui apenas 1 elemento` : `Array vazia`
        );
    }
...
export function remover2D(
    ...
    if (i > comprimento) {
        throw new Error(
            `Argumento inválido: ${i}, array possui ${comprimento} linha(s)`
        );
    }
